...
var startImport = function (message) {
  var deferred = Q.defer();
  var fileType, fileData, fileName,
    reportTitle = message.nameReport,
    reportDescription = message.descriptionReport,
    reportTlpLevel = message.tlpLevelReport,
    reportConfidence = message.confidenceReport,
    reportTags = message.tagsReport,
    collectionUuid = message.collectionUuid,
    reportPlaintext = message.plaintextReport;


  if (reportPlaintext) {
    createReport(null, null, null,reportTitle,reportDescription, collectionUuid, reportTlpLevel, reportConfidence, reportTags, reportPlaintext).then((response) => {
...
var createReport = (fileType, fileData, fileName,reportTitle,reportDescription, collectionUuid, reportTlpLevel, reportConfidence, reportTags, reportPlaintext) => {
  var deferred = Q.defer();
  API.createReport(fileType, fileData, fileName,reportTitle,reportDescription, collectionUuid, reportTlpLevel, reportConfidence, reportTags, reportPlaintext)
    .then(result => {
      console.log('in background result', result);
