...
const showSuccessScreen = function  (report) {
  APIKeyScreen.hidden = true;
  ImportScreen.hidden = true;
  ...
  if (report?.report_uuid) {
    let linkToReport = document.getElementById("link_to_report");
    let link = "https://" + C.SUB_DOMAIN + ".stixify.com/report/" + report.report_uuid + "/detail/";
    console.log(link);
    linkToReport.innerText =  link;
    linkToReport.setAttribute('href', link);
...
jQuery(document).ready(function($){
  $('#id_tag_tags_input').tagsInput({
    autocomplete: {
      source: ["api", "apt1", "apt28", "apt30", "demo", "example", "fancy-bear", "fireeye", "hell", "indicators"],
      // source: C.APP_URL + 'tags_input/autocomplete/SystemSetting/Tag/name/',
      autoFocus: true,
      delay: 20
    },
    ...
    // autocomplete_url: C.APP_URL + 'tags_input/autocomplete/SystemSetting/Tag/name/'
    ...
    if (tabURL.indexOf('http') > -1) {
      browser.tabs.sendMessage(tabs[0].id, {'is_blog': true}).then(function(contentResponse) {
        ...
        if (contentResponse.isContainRSS && C.APP_SWITCH === 'stixify') {
          Swal.fire("This looks like a blog. Use our other product, Obstracts, for automatic intelligence extraction of new posts. https://www.obstracts.com")
        }
  ...
  if (APIKey.length > 60 && APIKey.length < 91) {
    ...
    browser.runtime.sendMessage({validateAPIKey: true, APIKey: APIKey})
      ...
      if (response) { // window.globalSyncedStorage?.api_key && window.globalSyncedStorage?.api_key_valid
        ...
        showImportScreen();
        Swal.fire("Success. You can now start importing reports.");
...
const startImport = (event) => {
  ...
  let nameReport = document.getElementById('name_report')?.value;
  let descriptionReport = document.getElementById('description_report')?.value;
  let collectionSelectReport = document.getElementById('collection_select')?.value;

  let tlpLevelReport = document.getElementById('id_tlp_level')?.value;
  let confidenceReport = document.getElementById('id_confidence')?.value;
  let tagsReport = $('#id_tag_tags_input').val();
  let plaintextReport = $('#id_content:visible').val();
  ...
  // if (nameReport && collectionSelectReport) {
    ...
    let message = {
      nameReport,
      descriptionReport,
      collectionUuid: collectionSelectReport,
      tlpLevelReport,
      confidenceReport,
      tagsReport,
      plaintextReport,
    };
    message.createReport = true;
    ...
    browser.runtime.sendMessage(message)
      .then( function (response) {
        ...
        var reports = response.response.reports;
        // todo: hide preload
        console.log('response in sendMessage', response, reports);
